...
1. Read the full conversation once without naming any relational pattern.
2. Extract pattern-neutral clinical observations from the text (see observations).
3. Group observations into recurring themes across the conversation.
4. Only then assess which relational patterns are supported by multiple distinct themes.
5. For each included pattern, build evidence from observations already extracted in Step 2 — do not invent new points after labeling.
6. Write score and summary only after evidence is established.

## observations
Before naming any relational pattern, extract an array of pattern-neutral clinical observations from the full conversation.
...
- Do not name relational pattern labels or diagnostic categories.
...
- If no pattern reaches threshold, return {"observations": [...], "patterns": {}}
...
## evidence
For each included pattern, write evidence as an array of short clinical report lines drawn from your observations.
...
2) a brief clinical reading that states what this observation suggests about the client's relational pattern or interpersonal dynamics.
...
Prohibitions:
- Do not name relational pattern labels inside evidence.
